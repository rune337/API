...
def main():
    ...
    def process_account(account, iam):
        if args.user and (args.policy_arn or args.policy_name):
            ...
            targets = iamcli_common.get_config_list(account, 'attach_user_policies')

        if len(targets) == 0:
            print('ユーザーに追加するポリシーなし')
            return
        ...
        for target in targets:
            ...
            result = iamfunction.attach_user_policy(target['user'], policy_arn, iam)
            print(result['Note'])
