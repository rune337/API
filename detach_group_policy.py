...
def main():
    ...
    def process_account(account, iam):
        if args.group and (args.policy_arn or args.policy_name):
            ...
            targets = iamcli_common.get_config_list(account, 'detach_group_policies')

        if len(targets) == 0:
            print('グループから削除するポリシーなし')
            return
        ...
        for target in targets:
            ...
            result = iamfunction.detach_group_policy(target['group'], policy_arn, iam)
            print(result['Note'])
